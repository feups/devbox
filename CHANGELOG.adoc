...
Please ensure that `/etc/nixos/local-configuration.nix` does not contain any `zsh` settings (these settings have been moved to `configuration.nix`). The file should look like https://github.com/CIRB/devbox/blob/master/system/local-configuration.nix + your personal setting.
