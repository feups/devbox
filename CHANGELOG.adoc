...
`vagrant provision` don't override that file as it might contain personal settings. On the other hand, all changes in your local copy of the file (
in the root directory of Windows where the `Vagrant` file is located) is synchronized.

Please ensure that `local-configuration.nix` does not contain any `zsh` settings anymore (these settings have been moved to `configuration.nix`).
