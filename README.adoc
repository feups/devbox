...
In the host, pick a folder of your choice (where the `Vagrantfile` will sit) that will call `ROOT_DIR`. 

1. in `ROOT_DIR`, create a directory `ssh-keys`.
...
4. in `ROOT_DIR`, optionally copy the https://github.com/CIRB/devbox/blob/master/user/params.sh[params file] to automate your personal setup (your credentials, the clone of your stack, the git config,...). Do not copy/paste from Github UI, download source (https://raw.githubusercontent.com/CIRB/devbox/master/user/params.sh) and edit to avoid unwanted characters.
...
```
NOTE: Always verify that the cable of the VM is connected as virtualbox (mainly older versions but has appeared with 5.1.2 too) has a bug not starting the network interface. If so, you will have to *connect the cable*. See <<Troubleshooting>>
